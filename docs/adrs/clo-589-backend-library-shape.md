...
| `ClaudeBackend`, `FLAG_MATRIX` | Library, re-exported at the crate root | Only under `lokomotiv::backend::` | **Split.** `ClaudeBackend` is now re-exported at the crate root, matching its siblings (code moved). `FLAG_MATRIX` also had **code moved**: the `pub use codex::{CodexBackend, FLAG_MATRIX}` re-export was dropped so it is reachable only at `backend::codex::FLAG_MATRIX`, with `src/workflow.rs:128` updated to that path. It stays public because the binary reads it, but a Codex-specific flag table is not part of the provider-agnostic abstraction. |
