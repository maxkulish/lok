# PRD: Phase 2 — Predictable CLI Execution (v4)
...
| Supersedes | rev 3 (prd-phase-2-predictable-cli-execution-v3.md) |
...
| ID | Requirement | Priority | Acceptance Criteria |
|----|-------------|----------|--------------------|
| FR-20 | `step.model` reaches `Backend::query` via `StepContext.model` for every backend **and every call site**. **Today 8 call sites hardcode `None`:** `src/backend/mod.rs:381` (`run_query_with_config`), `src/conductor.rs:186`, `src/spawn.rs:173`, `src/spawn.rs:282`, `src/team.rs:82`, `src/team.rs:121`, `src/debate.rs:230`, `src/workflow.rs:2041` (synth backend post-debate). Workflow main paths (`src/workflow.rs:767, 778, 1179, 1737, 1943, 2171`) already thread `model_override`; verify they construct `StepContext.model` correctly under the new signature | Must | Snapshot test confirms `--model X` (Codex/Gemini), `model: X` (Ollama), profile (Claude), `modelId` (Bedrock); grep test asserts no `backend.query(..., None)` literal remains in `src/` outside test fixtures |
...
| `StepContext` migration cascades through every backend at once **and every caller** (conductor, spawn, team, debate, run_query_with_config, synth) | H | M | Single coordinated PR for trait + all backends + all callers; existing tests pin behavior; FR-20 audit-grep guards against regressions |
...
| 🔴 | Trait migration: `StepContext` param + `async fn health_check` + all 5 backends + all 13 call sites | Blocks FR-19a..f, FR-9..15a, and the entire per-step config / capability / multi-turn surface | FR-19a, FR-19b, FR-20 |
| 🟡 | Fix 8 non-workflow / synth call sites to construct `StepContext` with `step.model` | "Per-step config" is currently workflow-only; conductor/spawn/team/debate/synth all hardcode `None` | FR-20 |
...
- **Sequencing.** Land FRs in the order:
  ...
  3. **Trait & I/O hardening (FR-19a, FR-19b, FR-19c, FR-19d, FR-19e, FR-19f) + call-site sweep (FR-20)** — `StepContext` param on `Backend::query`, async `health_check` on the trait, all 5 backends migrated in lockstep, all 13 call sites updated. Single PR; grep test in CI to prevent regressions.
  ...
  8. **Per-step config plumbing (FR-21, FR-22, FR-23, FR-24, FR-24a)** — FR-20 already done in step 3.
...
| Codex `step.model` not threaded — **note: dead-code claim is wrong; workflow paths thread it** | FR-20 (non-workflow + synth call sites only) |
...
| 8 of 14 `Backend::query` call sites pass `None` for model | FR-20 (call-site sweep enumerated) |
...
3. Audit the 8 non-workflow / synth `Backend::query` call sites for FR-20 (see §11.9).
...
- [ ] **Enumerate every call site of `Backend::query`** (verified list, 14 sites):
  - **Pass `None` today (8 sites to fix per FR-20):** `src/backend/mod.rs:381`, `src/conductor.rs:186`, `src/spawn.rs:173`, `src/spawn.rs:282`, `src/team.rs:82`, `src/team.rs:121`, `src/debate.rs:230`, `src/workflow.rs:2041` (synth backend post-debate).
  ...
  - Plan the migration: one PR for the trait + all backends + all callers (FR-19a + FR-20). Grep test in CI to assert no `query(..., None)` literal in `src/` outside test fixtures.
