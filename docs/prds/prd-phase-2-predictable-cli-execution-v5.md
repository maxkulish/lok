# PRD: Phase 2 — Predictable CLI Execution (v5)
...
| Supersedes | rev 4 (prd-phase-2-predictable-cli-execution-v4.md) |

## Revision History

- **rev 1** — established schema/health/capability/per-step/usage/remote/session FR groups against the four CLI gap analyses.
...
- **O2 — Real health and version awareness.** `is_available()` reflects an actual ping (via cache populated by warmup); `lok doctor` reports each backend's installed version, auth method, and which optional flags are usable.
...
| `Backend::query` call sites that pass per-step `model` | 6 of 14 (workflow engine only) | 14 of 14 | Snapshot tests; grep proves no `query(..., None)` in `src/` outside test fixtures |
...
### FR Group: Backend Trait & Subprocess I/O Hardening (lands second; FR-25a lands first)

This group exists because several Phase 2 requirements (per-step model, multi-turn Ollama, safe multi-line prompts, async health) all require changing trait signatures and subprocess I/O patterns. Doing these together avoids two churn-passes. **Notes:**
- The codebase struct is named `Step` (defined in `src/workflow.rs:200`) — there is no `WorkflowStep`. All references in this PRD use `Step`.
- **`Backend::query` is already `async fn`** (`src/backend/mod.rs:235`, decorated with `#[async_trait]`). v3's FR-19b was incorrect on this point. The async work in this PR is therefore narrower: add `async fn health_check`, keep `is_available` sync but make it cache-only, and change `query`'s parameter list to take `StepContext`.
...
| FR-19b | `Backend` trait gains `async fn health_check(&self) -> Result<HealthStatus, BackendError>`. `Backend::query` stays `async fn` (already is). `Backend::is_available()` stays sync and returns the cached `HealthStatus.available` flag (see FR-9a / FR-10). **Correction from v3:** v3 framed this FR as making `query` async; in fact `query` already is — the new async surface is `health_check` only | Must | `health_check` exists on the trait; no `tokio::runtime::Handle::block_on` inside `src/backend/*.rs`; existing async behavior of `query` retained |
...
| FR-3a | Codex JSONL stream is parsed as discrete events, not by substring matching. The schema payload is extracted from the `agent_message` item of the **last** `turn.completed` event in the stream. Intermediate reasoning / tool-call items are discarded for the structured payload. `turn.failed` short-circuits to `BackendError::Backend` | Must | Unit test with a recorded JSONL fixture (captured per FR-40 **before** parser work begins) containing reasoning + tool-call + final agent_message; integration test confirms intermediate JSON-shaped reasoning is not mistaken for the result |
...
| ID | Requirement | Priority | Acceptance Criteria |
|----|-------------|----------|--------------------|
| FR-9 | `Backend` trait gains `async fn health_check(&self) -> Result<HealthStatus, BackendError>` (see FR-19b) | Must | `HealthStatus { available, version, auth_method, capabilities, unusable_flags, models: Vec<ModelInfo> }` |
...
| FR-11 | Ollama health check pings `GET /api/version` **and** `GET /api/tags`, reporting server version plus the list of locally available models. `/api/tags` returns `{"models": [{"name": "<tag>", "modified_at": "...", "size": <bytes>, "digest": "..."}]}` — `ModelInfo` must round-trip this shape | Must | Replaces unconditional `true`; `HealthStatus.models` populated with `ModelInfo { name, modified_at, size, digest }`; doctor renders model list |
...
| ID | Requirement | Priority | Acceptance Criteria |
|----|-------------|----------|--------------------|
| FR-20a | **Step sites — `step.model` reaches `Backend::query` via `StepContext.model` at every call site where a `Step` is in scope.** Workflow main paths (`src/workflow.rs:767, 778, 1179, 1737, 1943, 2171`) already thread `model_override`; verify they construct `StepContext.model` correctly under the new signature. The synth-backend site at `src/workflow.rs:2041` also has the `Step` in scope a few hundred lines up — pass `step.model_override` only when `synth_backend_name == step's primary backend`; otherwise fall back to `synth_config.model` | Must | Snapshot test confirms `--model X` (Codex/Gemini), `model: X` (Ollama), profile (Claude), `modelId` (Bedrock); grep test: `! rg -n 'step\..*query\(.*, None\)' src/workflow.rs` |
| FR-20b | **Non-Step sites — config-default / domain-struct sourcing for the 5 call sites that have no `Step` in scope.** (a) `src/backend/mod.rs:381` (`run_query_with_config`) — extend signature with `model: Option<&str>` and pass `config.defaults.model.as_deref()`; (b) `src/conductor.rs:186` (`execute_tool` → `query_backend`) — parse optional `input["model"].as_str()` from the JSON tool call; (c) `src/spawn.rs:282` (`AgentTask` execute closure) — add `model: Option<String>` to `AgentTask`; (d) `src/spawn.rs:173` (`plan_with_backend`) — use `self.config.defaults.model.as_deref()`; (e) `src/team.rs:82` and `src/team.rs:121` — add `model: Option<String>` to `BackendProfile`, thread it through; (f) `src/debate.rs:230` — look up `config.backends[backend.name()].model.as_deref()` for each round participant | Must | Per-site argv / payload snapshot tests; grep test: every remaining literal `None` in a `.query(...)` call in `src/` (excluding tests) must be on a line annotated with `// FR-20b: no model source` or removed |
...
This group exists because every FR above is testable only against a fixture that mirrors current backend output. Without these fixtures, regressions slip in silently. **FR-40 specifically must land *before* FR-3a parser work begins** — capturing a real JSONL stream is a prerequisite for writing the event parser.

| ID | Requirement | Priority | Acceptance Criteria |
|----|-------------|----------|--------------------|
| FR-40 | Repo ships a `tests/fixtures/codex/` directory with at least: a `turn.completed`-final JSONL stream, a `turn.failed` stream, a multi-turn stream with intermediate reasoning items, and a stream missing `agent_message` (older codex). Used to pin FR-3a parsing. **Capture procedure documented in PR** — `codex exec --json "prompt" > tests/fixtures/codex/<scenario>.jsonl`, scrubbed of any PII | Must | `cargo test backend::codex` consumes fixtures; **fixture exists on `main` before FR-3a parser PR opens** |
...
| Anthropic API endpoint + version header (`https://api.anthropic.com/v1/messages`, `anthropic-version: 2023-06-01`) still current | Lok user | To verify in §11.7 | If Anthropic deprecates 2023-06-01, Claude API mode breaks silently |
...
| `StepContext` migration cascades through every backend at once **and every caller** (conductor, spawn, team, debate, run_query_with_config, synth) | H | M | Single coordinated PR for trait + all backends + Step-aware callers (FR-20a); follow-on PR for non-Step callers (FR-20b) once the `StepContext` shape is settled; existing tests pin behavior; per-file grep tests guard against regressions |
| `BedrockBackend` is feature-gated (`#[cfg(feature = "bedrock")]`) and may have drifted against the current trait | L | M | §11.8 step: `cargo check --features bedrock` before trait-change PR; coordinate the migration with Bedrock's existing async-constructor pattern |
...
### What to Fix First (Priority Snapshot)

| Priority | Action | Why | FR |
|----------|--------|-----|----|
| 🔴 | Add `usage: Option<TokenUsage>` to `StepResult` (stand-alone PR) | 3 backends already populate `QueryOutput.usage`; data silently dropped today. Tiny diff; unblocks FR-25..30 immediately | FR-25a |
| 🔴 | Trait migration: `StepContext` param + `async fn health_check` + all 5 backends + all 13 call sites | Blocks FR-19a..f, FR-9..15a, and the entire per-step config / capability / multi-turn surface | FR-19a, FR-19b, FR-20a |
| 🟡 | Fix the 5 non-Step call sites by adding `model: Option<String>` to `AgentTask` and `BackendProfile`, and threading `config.defaults.model` into `run_query_with_config` and `plan_with_backend` | "Per-step config" is currently workflow-only; conductor/spawn/team/debate hardcode `None` and have no `Step` in scope, so they need an alternate model source | FR-20b |
| 🟡 | Capture Codex JSONL fixtures into `tests/fixtures/codex/` before writing the event parser | No test data exists; FR-3a is untestable without this | FR-40 |
| 🟡 | Decide Claude per-mode capability matrix (Api vs Cli) | Must happen before capability registry is built; influences FR-16 / FR-17 routing | FR-13a, FR-16 |
| 🟢 | Pin `/api/tags` response shape into `ModelInfo` struct | Needed before Ollama health-check / model verification land | FR-11 |
| 🟢 | Verify Anthropic API endpoint + `anthropic-version` header still current | Avoids silent breakage during Phase 2 implementation | §11.7 |

### Release Plan

- **Sequencing.** Land FRs in the order:
  1. **Phase 1 verification gate** (no FR; confirm landed work).
  2. **`StepResult.usage` (FR-25a) — stand-alone PR.** Smallest possible change. 3 of 5 backends start reporting usage end-to-end immediately. No trait churn yet; conductor copy is a 5-line addition. This is the proof-of-concept that the rest of the migration plan can hang on.
  3. **Trait & I/O hardening (FR-19a, FR-19b, FR-19c, FR-19d, FR-19e, FR-19f) + Step-site sweep (FR-20a)** — `StepContext` param on `Backend::query`, async `health_check` on the trait, all 5 backends migrated in lockstep, all 8 Step-aware call sites (6 main workflow paths + synth) updated. Single PR; per-file grep tests in CI to prevent regressions.
  3b. **Non-Step site sweep (FR-20b)** — small follow-on PR: add `model: Option<String>` to `AgentTask` and `BackendProfile`; extend `run_query_with_config` signature; thread `config.defaults.model` at `spawn::plan_with_backend` and `debate::round`; parse `input["model"]` at the `query_backend` conductor tool. Lands after FR-20a so the `StepContext` shape is settled.
  4. **`TokenUsage` extension (FR-25b)** — needed before Codex/Anthropic usage extraction picks up cached / reasoning fields.
  5. **Codex JSONL fixture capture (FR-40)** — lands ahead of the parser PR.
  6. **Health checks + warmup (FR-9, FR-9a, FR-10, FR-11, FR-11a, FR-12, FR-13, FR-13a, FR-14, FR-15, FR-15a)**.
  7. **Capability registry (FR-16, FR-17, FR-18, FR-19)**.
  8. **Per-step config plumbing (FR-21, FR-22, FR-23, FR-24, FR-24a)** — FR-20a already done in step 3, FR-20b in step 3b.
  9. **Ollama remote/options/tools/think (FR-31, FR-32, FR-33, FR-33a, FR-33b, FR-34, FR-35, FR-36)**.
  10. **Schema output per backend (FR-1, FR-2, FR-3, FR-3a, FR-3b, FR-4, FR-5, FR-6, FR-7, FR-8)**.
  11. **Usage extraction beyond regression-pin (FR-25, FR-26)** + run-summary aggregation (FR-28, FR-29, FR-30).
  12. **Session hygiene + CI cleanliness (FR-37, FR-37a, FR-37b, FR-37c, FR-38, FR-39)**.
  13. **Remaining integration test fixtures (FR-41, FR-42, FR-43)** — land alongside whichever FR consumes them.
...
- **After step 2 (FR-25a) lands.** Run an existing Ollama workflow and confirm `lok run --output json` now includes `step.usage` for the Ollama and Claude API steps; verify `cargo test` is clean and rs-wisper's deserialization of `StepResult` still works.
...
| Codex `step.model` not threaded — **note: dead-code claim is wrong; workflow paths thread it** | FR-20a (Step-aware sites, incl. synth) + FR-20b (non-Step sites: spawn/team/debate/conductor/run_query_with_config) |
...
| Ollama single-turn only / drops history (ollama-api §1) — **architectural, not "medium"** | FR-19c |
...
| Ollama usage already extracted in code (`src/backend/ollama.rs:155`) | FR-27 (regression pin) + FR-25a (conductor copy) |
...
| Claude dual-mode capabilities (`ClaudeMode::{Api, Cli}`) — **investigations missed this** | FR-13a, FR-16 (per-mode capability sets) |
...
| All backends: no capability advertising (ollama-api §5.3, gemini §5.5, codex §4.3) | FR-16, FR-17, FR-18, FR-19 |
| All backends: `Backend::query` takes bare strings, no `StepContext` or history | FR-19a, FR-19c |
| `Backend::query` is already `async fn` (`src/backend/mod.rs:235`) — v3 wrongly framed this as new work | FR-19b (corrected) |
| 8 of 14 `Backend::query` call sites pass `None` for model | FR-20a (3 Step-aware sites incl. synth) + FR-20b (5 non-Step sites) |
| `StepResult` has no `usage` field (`src/workflow.rs:897`) — **investigations missed this** | FR-25a |
...
### 11.0a Recommended Walk Order

Walk in this order — earlier items unblock later ones, and the order matches §9 Release Plan:

1. Read this PRD top-to-bottom (skip v3 once you're confident this revision is the current spec).
2. Confirm `StepResult` shape at `src/workflow.rs:897` and the absence of a `usage` field → plan the FR-25a stand-alone PR first.
3. Audit the 8 non-workflow / synth `Backend::query` call sites and bucket them into Step-aware (FR-20a) vs no-Step-in-scope (FR-20b). See §11.9 and the rev-5 site-by-site table.
4. `cargo check --features bedrock` to confirm `BedrockBackend` still compiles against the current trait before the trait-migration PR.
5. Verify Claude API endpoint URL + `anthropic-version` header (§11.7) — quick HEAD request to `https://api.anthropic.com/v1/messages`.
6. Capture a real Codex JSONL stream into `tests/fixtures/codex/` (§11.4) — prerequisite for FR-3a parser work.
7. Confirm Ollama `/api/tags` response shape against the `ModelInfo` design (§11.6).

### 11.1 `src/backend/mod.rs` — Trait Signatures & Token Usage
- [ ] **Confirm current `Backend::query` signature**: `async fn query(&self, prompt: &str, cwd: &Path, model: Option<&str>) -> Result<QueryOutput, BackendError>` (verified at `src/backend/mod.rs:235`, decorated `#[async_trait]`). It is **already async**. Plan the exact diff to take `StepContext` (FR-19a) — this is a parameter-list change, not an async-ness change.
- [ ] **Confirm current `is_available` is sync** `fn is_available(&self) -> bool` (verified at `src/backend/mod.rs:241`). Today `OllamaBackend` returns `true` unconditionally — refactor cost is real. Plan FR-10 cache-only behavior.
...
- [ ] **Audit `run_query_with_config`** (`src/backend/mod.rs:381`): currently `backend.query(&prompt, &cwd, None)`. No `Step` in scope — FR-20b fix is to extend the signature with `model: Option<&str>` and pass `config.defaults.model.as_deref()` from the caller.
...
- [ ] **Locate `StepResult`** (`src/workflow.rs:897`). Confirmed shape: `{ name, output, parsed_output, success, elapsed_ms, backend, raw_output, stderr, exit_code, validation, failure }`. **Missing `usage` (FR-25a) and `rendered_prompt` (FR-19f)** — both fields must be added before usage observability or multi-turn history can function. **FR-25a lands as a stand-alone PR first** per §9 sequencing.
- [ ] **Confirm the conductor's step iteration loop.** Identify the six known workflow `Backend::query` call sites (lines 767, 778, 1179, 1737, 1943, 2171) that already pass `model_override`, plus the synth-internal site at line 2041 that hardcodes `None`, plus the seven non-workflow callers (conductor.rs:186, spawn.rs:173+282, team.rs:82+121, debate.rs:230, mod.rs:381). 14 call sites total; 8 of them pass `None` today. The Step-aware sites (7 main + 1 synth) get the `StepContext` migration in FR-20a; the 5 non-Step sites get config-default / domain-struct sourcing in FR-20b.
...
- [ ] **Locate the existing `line.contains("\"agent_message\"")` substring match.** Document every event type currently observed in production JSONL streams (capture a real stream into `tests/fixtures/codex/<scenario>.jsonl` per FR-40 **before** writing the event-driven parser — this fixture is the test bed).
...
- [ ] **Locate the `/api/chat` request body.** Confirm it sends only `{model, messages: [{role: "user", content: prompt}], stream: false}` (verified at `src/backend/ollama.rs:18–23`); plan addition of `system`, `options`, `format`, `keep_alive`, `tools`, `think` (FR-5, FR-19c, FR-33, FR-33a, FR-33b, FR-34).
...
- [ ] **Confirm `/api/tags` response shape** before adding `ModelInfo`. Current Ollama returns `{"models": [{"name": "<tag:version>", "modified_at": "<rfc3339>", "size": <bytes>, "digest": "<sha256>"}]}`. Pin `ModelInfo { name, modified_at, size, digest }` to round-trip this shape; serde test against the recorded fixture from FR-42.
...
- [ ] **Sanity-check the Anthropic API contract.** Endpoint `https://api.anthropic.com/v1/messages` (verified at `src/backend/claude.rs:143`); `anthropic-version: 2023-06-01` header (verified at `src/backend/claude.rs:145`). Before Phase 2 implementation begins, confirm against Anthropic's current docs that this version header is still supported; if a newer header is required, the Claude API mode breaks silently and the fix belongs in the same PR as FR-25a's regression test.
...
- [ ] **Run `cargo check --features bedrock` before the trait-change PR** to ensure Bedrock still compiles against the current trait. If it has drifted (the feature may not be exercised on every CI run), reconcile in the trait-change PR so all five backends migrate together.
- [ ] **Confirm `with_usage` populated at `src/backend/bedrock.rs:199`.** FR-27a regression pin applies.
- [ ] **Confirm Bedrock's async constructor pattern** (`async fn new`, `src/backend/bedrock.rs:91`) — when refactoring backend construction, preserve this; do not collapse it into a sync `new`.
...
### 11.9 Cross-Cutting
- [ ] **Enumerate every call site of `Backend::query`** (verified list, 14 sites):
  - **Pass `None` today (8 sites split between FR-20a and FR-20b):**
    - **FR-20a (Step in scope — 3 sites):** `src/workflow.rs:2041` (synth backend; use `step.model_override` when synth backend matches step's primary, else `synth_config.model`). Note: the 6 workflow main paths at 767/778/1179/1737/1943/2171 already thread `model_override` and are listed here for completeness, not for the None-fix sweep.
    - **FR-20b (no Step in scope — 5 sites):**
      - `src/backend/mod.rs:381` — extend `run_query_with_config(...)` signature with `model: Option<&str>`; pass `config.defaults.model.as_deref()`.
      - `src/conductor.rs:186` — parse optional `input["model"].as_str()` from the `query_backend` tool's JSON input.
      - `src/spawn.rs:173` (`plan_with_backend`) — use `self.config.defaults.model.as_deref()`.
      - `src/spawn.rs:282` — add `model: Option<String>` to `AgentTask`; pass through.
      - `src/team.rs:82`, `src/team.rs:121` — add `model: Option<String>` to `BackendProfile`; thread through.
      - `src/debate.rs:230` — look up `config.backends[backend.name()].model.as_deref()` for each round participant.
  - **Already pass `model_override` (6 sites to migrate to `StepContext.model` under the new signature):** `src/workflow.rs:767, 778, 1179, 1737, 1943, 2171`.
  - Plan the migration: one PR for the trait + all backends + Step-aware callers (FR-19a + FR-20a); follow-on PR for non-Step callers (FR-20b). Per-file grep tests in CI: `! rg -n 'step\..*query\(.*, None\)' src/workflow.rs` (FR-20a) and an explicit allow-list of remaining `None` literals at FR-20b sites annotated with `// FR-20b: no model source`.
...
- [x] **Confirm `StepResult` serialization surface (closed in rev 5).** `StepResult` at `src/workflow.rs:897` derives only `Debug, Clone` — no `Serialize`/`Deserialize`. `format_results` at `src/workflow.rs:3567` is text-only. Conclusion: `StepResult` is consumed as a Rust struct (rs-wisper via crate dependency), not over JSON. The new `usage` and `rendered_prompt` fields are additive at the struct level — `Option` is sufficient; no `#[serde(default)]` coordination required. Still land FR-25a first so the conductor-copy pattern lands in isolation before the larger trait migration touches every call site.
